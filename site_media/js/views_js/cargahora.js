...
$(document).ready(function(){
	...
	//--Al hacer click al sol
	$("#sol").click(function(e){
		e.preventDefault();
		$("#sol").addClass("btn-primary");
		$("#sol").addClass("opcion_activa");
		$("#luna").removeClass("opcion_activa");
		$("#texto_diurno").css({"font-weight":"bold"});
		$("#texto_nocturno").css({"font-weight":"normal"});
		$("#luna").removeClass("btn-primary");
		eliminar_grupo_filas_todas();
		cargar_filas();
		//valores_sol();
	});
	//--al hacer click a la luna
	$("#luna").click(function(e){
		e.preventDefault();
		$("#luna").addClass("btn-primary");
		$("#luna").addClass("opcion_activa");
		$("#sol").removeClass("opcion_activa");
		$("#texto_diurno").css({"font-weight":"normal"});
		$("#texto_nocturno").css({"font-weight":"bold"});
		$("#sol").removeClass("btn-primary");
		eliminar_grupo_filas_todas();
		cargar_filas();
		//valores_luna();
	});
			...
			//--	
				...
				$.ajax({
								...
								success:function(resp){
									...
									if(recordset[0]==4){
										carga_pnoty_danger("Error","Ya existe un registro de horas extras para el mes seleccionado");
									}else
									if(recordset[0]==1){
	...
	$("#modal_reporte_aceptar").click(function(e){
		...
		$.ajax({
					...
					success: function(resp){
						...
						else if(recordset[0]==7){
								carga_pnoty_info("Informaci&oacute;n","Ya existe un registro desaprobado para este mes y este trabajador");
						}
						else if(recordset[0]==5){
							carga_pnoty_info("Informaci&oacute;n","No puede modificar el registro");
						}
						else{
...
function cargar_filas(){
	...
	$.ajax({
				...
				success:function(resp){
					...
					//--
					//Para asignar valores por defecto a horas....
					var unidad_administrativa = $("#unidad_administrativa").val();
					//SI ES DIURNA
				    if($("#sol").hasClass("opcion_activa")){
				    	if ((unidad_administrativa==3) || (unidad_administrativa==4) || (unidad_administrativa==5)){
				    		limite_inicial = 300;//05:00 AM
				    		limite_final = 540;//09:00 AM
				    		valor_inicial = "05:00";
				    		valor_final = "9";
				    		//alert(limite_final);
				    	}else{
				    		limite_inicial = 990;//04:30
				    		limite_final = 1140; //19000
				    		valor_inicial = "16:30";
				    		valor_final = "19";
				    	}
				    
				    }else //SI ES NOCTURNA
				    if($("#luna").hasClass("opcion_activa")){
				    	limite_inicial = 1140;//19:00
				    	limite_final = 1440;//24:00
				    	valor_inicial = "19:00";
				    	valor_final = "24";
				    }
					//--
				    ...
				    $('#hora_ini'+valore).val(valor_inicial);
				    //alert($('#hora_ini'+valore).val());
				    $('#hora_ini'+valore).timepicker({
				    	showMeridian	: false,
				    	maxHours : valor_final,
					    ...
					    if(m<limite_inicial)
					        $('#hora_ini'+valore).timepicker('setTime', valor_inicial);
					});
				    $('#hora_fin'+valore).timepicker({
				    	showMeridian : false,
				    	maxHours : valor_inicial,
				    }).on('changeTime.timepicker', function(e) {
					    var h= e.time.hours;
					    var m= e.time.minutes;
					    var mer= e.time.meridian;
					    ...
					    //615 minutes = 10h*60m + 15min
					    if(m<limite_inicial)
					        $('#hora_fin'+valore).timepicker('setTime', valor_inicial);
					    if(m>limite_final){
					    	//alert("Here a la mil");
					        $('#hora_fin'+valore).timepicker('setTime', valor_inicial);
					    }
...
function eliminar_filas(){
	...
	if (valore>1){
		...
		$("#cuerpo_horas_extras"+valore).fadeOut(300, function() { $(this).remove(); });
	}
	//--
}
...
//--Eliminar grupos de filas todas
function eliminar_grupo_filas_todas(){
	var valore = $('.fila_he').size();
	for(i=valore;i>0;i--){
		$("#cuerpo_horas_extras"+i).remove();
	}
...
function validar_filas(){
	...
	for(i=0;i<=cantidad;i++){
		...
		if($("#motivo"+x).val()==""){
			carga_pnoty_info("Informaci&oacute;n","Debe señalar  de la fila #"+x);
			return false;
		}
	}
}
...
function btn_seleccionar_registro(caja){
	...
	if(arreglo_datos[8]==1 || arreglo_datos[8]==5){
		...
		$("#sol,#luna").attr("disabled","disabled");
		subir_top();	
